...
Search the docs
GuidesProduct manualsGlossaryReferenceSamples Docker v17.09 (current) 
...
echo "It’s OK if apt-get reports that none of these packages are installed.
...
echo "For Ubuntu 16.04 and higher, the Linux kernel includes support for OverlayFS, and Docker CE will use the overlay2 storage driver by default.
...
echo "This command downloads a test image and runs it in a container. When the container runs, it prints an informational message and exits.
