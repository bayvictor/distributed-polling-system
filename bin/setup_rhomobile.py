...
"""
Written by Victor Huang, for easier interactively install, setup for rhomobile!
From now on use python script instead of bash-shells for instruction!
timestamp:
2010-10-29-03-04-50
...
#--------+---------+---------+---------+---------+---------+---------+---------+
# FileTest - class for testing filenames
#
class FileTest:
    """
    Perform various file-related tests.

    This module has been written to isolate some system dependencies when
    porting code between regular Python and Jython.
    """
    # End of FileTest

def exists( DirPath, FileName ):
    """
    Test if named file in specified directory exists, either as a file
    or as a directory.
    
    Parameters:
      DirPath       path of directory where file resides, or None
      FileName      name of file to be tested
    
    Returns:
      A fully qualified filename string if the file exists, otherwise
      None.
    """
    if not DirPath:
        DirPath = os.getcwd()
...
def isFile( DirPath, FileName ):
    """
    Test if named file in specified directory exists as a file.
    ...
    Returns:
      ...
      None.
    """
    FullName = exists( DirPath, FileName )
    if FullName:
        if os.path.isfile( FullName ): return FullName
    return None

def isDirectory( DirPath, FileName ):
    """
    Test if named file in specified directory exists as a directory.
    ...
    FullName = exists( DirPath, FileName )
    if FullName:
        if os.path.isdir( FullName ): return FullName
    return None

def canRead( DirPath, FileName ):
    """
    Test if named file in specified directory can be read.
    ...
    Returns:
      A true value if the indicated file can be read by the calling program,
      otherwise a false value.
    """
    FullName = exists( DirPath, FileName )
    if FullName:
        ### if checkMode( FullName, 0x124 ): return FullName
        if os.access( FullName, os.R_OK ): return FullName
    return None

def canWrite( DirPath, FileName ):
    """
    Test if named file in specified directory can be written or created.
    ...
    Returns:
      A true value if the indicated file can be written or created by the 
      calling program, otherwise a false value.
    """
    FullName = exists( DirPath, FileName )
    if FullName:
        ### if checkMode( FullName, 0x092 ): return FullName
        if os.access( FullName, os.W_OK ): return FullName
    else:
        return canCreate( DirPath, FileName )
    return None

def canCreate( DirPath, FileName ):
    """
    Test if named file in specified directory can be created.
    ...
    Returns:
      A true value if the indicated file can be created by the 
      calling program, otherwise a false value if the file already exists
    ...
    #### if checkMode( DirName, 0x092 ): return FullName
    if os.access( DirName, os.W_OK ): return FullName
    return None

def checkMode( FullName, Mode ):
    """
    Local helper function checks file access mode against supplied value:
    returns true value if any of the required mode bits are set,
    otherwise false.
    """
    Stat = os.stat( FullName )
    FileMode = S_IMODE( Stat[ST_MODE] )
    sys.stdout.write( "    Mode for "+FullName+" is "+hex(FileMode)+"\n" )
    # For now:
    # base writeability assessent on union of owner, group, world permissions
    return FileMode & Mode

#--------+---------+---------+---------+---------+---------+---------+---------+
#
# Stand-alone test code follows:
#



def wgetIfNotExists ( PATH, FILE ) :
    e1 = exists( PATH, FILE )
    if not   e1:
				os.system("wget " + PATH  + FILE )
    elif e1 != FILE:
				sys.stdout.write( "exists( '.', "+FILE+" ) returned '"+e1+"'\n" )
				

def ask_input() : 
		ch = raw_input( "Press ^C to break, \'b\' to bypass next step, any other key to continue." )
#		ch = ch.lower()    
		return ch

ch=' '
print "Automatic Installation for rhomobile on ubuntu linux, or any system\n"
print "which has \"apt-get\" already installed. "
if os.getuid() != 0:
	print "This script must be run as root."
	print "Type \'man sudo\' for more details."
	sys.exit(0)

print "BuildingRhodes\
...
ch = ask_input()
print """8 HOWTO: Customize application name and icon
...
Required Software"""
ch = ask_input()
RUBY_FILE="ruby-1.9.2-p0.tar.gz"
RUBY_PATH=" ftp://ftp.ruby-lang.org//pub/ruby/1.9/"
print "Ruby 1.8.6 or 1.8.7 - http://www.ruby-lang.org/en/downloads/ . For windows use ruby installer"
wgetIfNotExists(RUBY_PATH,RUBY_FILE) #
os.system("tar -zxvf  " + RUBY_FILE )
os.system("cd ruby-1.9.2-p0 ")
os.system("./configure ")
os.system("make ")
os.system("sudo make install ")
ch = ask_input()
print """RubyGems (v1.3.7 or higher) - http://rubygems.org/
...
ch = ask_input()
#os.system("wget http://production.cf.rubygems.org/rubygems/rubygems-1.3.7.tgz")
wgetIfNotExists( "http://production.cf.rubygems.org/rubygems/", "rubygems-1.3.7.tgz")
os.system("tar -zxvf rubygems-1.3.7.tgz")
os.system("./configure")
os.system("make")
os.system("sudo make install")

print "rhodes gem - run \"[sudo] gem install rhodes\" to install the latest rhodes gem"
ch = ask_input()
os.system("sudo gem install rhodes")
print """JDK version 1.6.0_2 or higher. (recommended version 6.0 update 11) - required for Android & Blackberry
 1.6.0_13 Mac OSX 10.5 (Install then run \"Java Preferences\" utility to change default version)"""
ch = ask_input()
os.system("sudo apt-get install -y openjdk-6-jdk default-jdk")

print """ Sun JDK For Windows/Linux NOTE: The 32-bit Windows version is required for Blackberry tools.
Make sure you set JAVA_HOME and that JAVA_HOME/bin is on your path."""
ch = ask_input()
print "JAVA_HOME=\"$JAVA_HOME\""
print "PATH=$PATH"
ch = ask_input()
os.system("sudo apt-get install -y  lib32ncurses5 lib32z1 libc6-i386 libc6-dev-i386 lib32gcc1  lib32stdc++6")

print """Setup HOME environment variable:
...
ch = ask_input()
os.system(" git clone git://github.com/rhomobile/rhodes.git")
os.system(" cd rhodes")
ch = ask_input()
print "(Optional) Checkout the branch you are interested in (if not master):"
ch = ask_input()
os.system(" git checkout -f -b <branchname> --track origin/<branchname>")
print "Make sure you\'re up-to-date on the branch:"
os.system(" git pull")
print """Add the bin folder to your path. In linux/mac it is typically done via the .profile. In windows it is done via the system control panel.
...
ch = ask_input()
os.system(" rhodes-setup")
print "Build rhodes gem from source\
You don't have to do it if you cloned sources, but if you want to build rhodes gem then do following:"
ch = ask_input()
os.system(" do clean clone")
os.system(" cd rhodes")
os.system(" rake gem")
os.system(" gem install rhodes-[version].gem")
print "Use Rhodes sources directly (without gem)\
Set sdk: property in the build.yml (it is in the root folder of your app) to the Rhodes source code root folder."
ch = ask_input()

print "!!!!!! iPhone  !!!!!!!"
ch = ask_input()
print """iPhone versions of Rhodes apps need to be built on Mac machines. The instructions below describe how to do this from either the XCode development environment or from the command line.

Prerequisites
Mac OS X 10.5 (Leopard) or greater (Snow Leopard)
Follow these instructions:"""
ch = ask_input()
print "iPhone SDK installed (we recommend the latest stable version available from the iPhone Dev Center"
ch = ask_input()
print """Build Application with XCode
...
os.system(" rake switch_app")
print """This changes your rhobuild.yml in the SDK folder to point to your application.
...
print """After press Yes in dialog 'Would you like to create a new key pair file' - nothing appear. No dialogs, no error messages just nothing: this is a problem with Java SDK and Blackberry SDK versions
