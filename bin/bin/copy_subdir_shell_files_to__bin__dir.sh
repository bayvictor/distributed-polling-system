...
find|grep -e "\.sh$" | grep  -v "$1" | grep  -v "$2"  > list_src_sh_files.txt

cat list_src_sh_files.txt | sed 's/^/norm.sh /g' | sed -e 's/\.\///g' > test.txt.sh 
chmod +x *.sh
./test.txt.sh  |sed 's/^/~\/bin\//g' > test022.txt
...
echo "^C to edit \"/tmp/go.sh\" and manually run it, or any other key to copy all files to ~/bin"
read readline
