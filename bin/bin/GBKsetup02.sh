...
sudo apt-get install -y --force-yes bison g++-multilib git gitk gperf libxml2-utils flex -y
# for linux menuconfig
sudo apt-get install -y --force-yes libncurses5-dev fakeroot libswitch-perl -y

echo "3. Install Emacs"
sudo apt-get install -y --force-yes emacs23 mew -y
if [ ! -e ~/.emacs.d ]; then
    mkdir ~/.emacs.d
fi
...
echo "4. Install Vim"
cd ~
sudo apt-get install -y --force-yes vim vim-gui-common exuberant-ctags -y
if [ ! -e .vim/vimrc ]; then
    rm -rf .vim/ .vimrc
...
sudo apt-get install -y --force-yes thunderbird thunderbird-locale-zh-cn thunderbird-locale-zh-tw thunderbird-locale-ko thunderbird-locale-ja -y

echo "6. Install filezilla"
sudo apt-get install -y --force-yes filezilla -y

echo "7. Install archive tools"
sudo apt-get install -y --force-yes unar -y
sudo apt-get install -y --force-yes convmv -y  # convert the file name coding: convmv -f UTF-8 -t GBK --notest utf8

echo "8. Install System tools"
sudo apt-get install -y --force-yes ckermit tweak samba smbclient ssh -y
...
sudo apt-get install -y --force-yes cups-pdf -y #pdf printer
sudo apt-get install -y --force-yes mupdf apvlv -y #pdf reader like vi control
sudo apt-get install -y --force-yes uzbl mutt -y # uzbl: Browser, mutt: Mail client
sudo apt-get install -y --force-yes gparted -y # gparted: A disk manage software
sudo apt-get install -y --force-yes ecryptfs-utils -y # ecryptfs

echo "11. Install Fcitx Input Method"
sudo apt-get install -y --force-yes im-switch fcitx fcitx-config-gtk fcitx-table-all -y
im-switch -s fcitx -z default
# exchange the caps lock and control key
dconf write /org/gnome/desktop/input-sources/xkb-options "['ctrl:swapcaps']"

echo "12. Set some misc"
sudo apt-get install -y --force-yes tree tmux -y
...
sudo apt-get install -y --force-yes gtk2-engines-pixbuf -y # remove warning when open GTK software in shell
sudo apt-get install -y --force-yes gconf-editor -y # used to set the app which use the gtk library, such as iptux
sudo apt-get install -y --force-yes gstreamer1.0-libde265 -y # H265 Video play
...
# replace ia32-libs
sudo apt-get install -y --force-yes lib32z1 lib32ncurses5 lib32bz2-1.0 -y
# install -y --force-yes old version ia32-libs
cd ~/Backup/documents
...
sudo apt-get install -y --force-yes libappindicator1 -y
...
#sudo apt-get install -y --force-yes unity-control-center-signon unity-control-center-unity
