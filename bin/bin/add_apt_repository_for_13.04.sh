#  install -y  the add-apt-repository tool (if necessary)
...
echo " Ubuntu 12.4  install -y  notes
Also related with  install -y  of Ubuntu 12.04 is my other post  Ubuntu 12.04 review on Lenovo Q150.
...
"
sudo -E wget --output-document=/etc/apt/sources.list.d/medibuntu.list http://www.medibuntu.org/sources.list.d/$(lsb_release -cs).list && sudo apt-get --quiet update && sudo apt-get --yes --quiet --allow-unauthenticated  install -y  medibuntu-keyring && sudo apt-get --quiet update

    sudo apt-get --yes --quiet --allow-unauthenticated  install -y  medibuntu-keyring && sudo apt-get --quiet update

    sudo apt-get  install -y  app- install -y -data-medibuntu apport-hooks-medibuntu 

    sudo apt-get  install -y  non-free-codecs ubuntu-restricted-extras libdvdcss2



echo "For google applications add one by one, the apps will add the repositories of the apps you  install -y .
http://www.google.com/linuxrepositories/

Flash:
"
    sudo apt-get  install -y  flashplugin- install -y er


echo "Minitube and Vlc:
"
    sudo apt-get  install -y  minitube vlc
...
echo "
For google applications add one by one, the apps will add the repositories of the apps you  install -y .
http://www.google.com/linuxrepositories/

Flash:
 On an APT-based system (Debian, Ubuntu, etc.), download the key and then use apt to  install -y  it.
"
