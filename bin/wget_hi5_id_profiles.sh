...
IDFILE=$IDFILE".out" 
OUTFILE1=$IDFILE".out1"
OUTFILE2=$IDFILE".out2"
OUT_SH=`timestamp`."$IDFILE"."$BROW".sh

cat $IDFILE | sed 's|$|\.html|g;s|$|;sleep $[ ( $RANDOM % $DELAY )  + 1 ]s; |g;' >>$OUTFILE2
...
cat $IDFILE | sed 's|^|wget  --load-cookies=NathanH.txt  http://www.hi5.com/apps/pets.html?dataSource=Pets&ll=nav#/pet/|g;s|$| -O |g' >>$OUTFILE1

paste $OUTFILE1 $OUTFILE2 > $OUT_SH

#;sleep $[ ( $RANDOM % $DELAY )  + 1 ]s; |g;' >>$OUTFILE1
