...
def do_para_block_for_lines( lines, worker_num, out_fn ):
  ...
  sym_lines =   get_neat_lines(dns_fn);

  out_fn =  dns_fn+".out"
  try:
    fout=open( out_fn, "w")
  except:
    print ( "can't open file ({}) for writing".format ( out_fn ) )

  for line in sym_lines:
    for key in dns2ip_dict:  # when out of ii loop
      line.replace ( key , dns2ip_dict[key] )
...
if __name__ == '__main__':
  ...
  do_para_block_for_lines( url_lines, worker_num, dns_fn ) ## finishing all dns2ip mapping
