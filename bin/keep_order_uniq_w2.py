...
def parse_file(fn):
  ...
  dict_out = {};
  dict_out["w1"]=[];    dict_out["w2"]=[];    dict_out["w3"]=[];  
  keep_order_list=[]
  cmd4 ="  cat "+ fn + "| tr ' ' '\n'"
  p4 = os.popen( cmd4 )
  lines = p4.readlines()
  ...
  fout=open( fn_out, "w")
  w2fn_out =  fn+".keeporder.uniq.w2.txt"
  w2fout=open( fn_out, "w")
  total_lines = len(lines)
  for ii in range(0, len(lines)):
    testwords, newline = split_with_nonalnum(lines[ii])
    words=newline.split()
    if len(words) <= 0:
      continue
    for mm in range(0, len(words)):
      print "mm=("+str(mm)+"),line=("+lines[ii]+")"
      print words
