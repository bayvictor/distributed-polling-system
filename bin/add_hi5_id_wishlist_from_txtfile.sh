...
#adding wish list from command line using the input current up & running browser $BROW -- the browser
cat $IDFILE | sed 's|^|http://www.hi5.com/apps/pets.html?dataSource=Petsll=nav#/pet/|g;s/^/$BROW /g; s|$|;sleep $[ ( $RANDOM % $DELAY )  + 1 ]s; |g;' >>$OUT_SH


## commenting out 2 lines original bring up browsers
