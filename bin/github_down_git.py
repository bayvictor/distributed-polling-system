...
#  for kk in range(0, len(lines) ):
  ...
  if 0 != write_parsed_link:
    fout.close()
  if True == gitonly:
    os.popen( "chmod +x *.sh")  ## run git command line by line!  
...
"""
def get_github_page0(keyword,per4s="20",per1s="15", random_s="30", max_pages=2, gitonly=True ): ## True==git down the code!
...
if __name__=="__main__":
  ...
  if len(sys.argv) < 5: 
    print "Format: "+sys.argv[0]+ " <keyword> <per_url_sleep_second> <per_4_urls_sleep_second> <max_random_wait_second> <gitonly>"
