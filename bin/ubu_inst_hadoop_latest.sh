...
export WGETDIR=~/src/_hadoop_hbase
#export WGETDIR=/home/vhuang/src/_hadoop_hbase

mkdir $WGETDIR
# please edit all 4 lines as below for new releases of hadoop/hbase
export HBA=hbase-0.90.0
export HBA_URL=http://people.apache.org/~stack/hbase-0.90.0-candidate-2/hbase-0.90.0.tar.gz
export HAD=hadoop-0.21.0
export HAD_URL=http://mirror.cloudera.com/apache/hadoop/core/hadoop-0.21.0/hadoop-0.21.0.tar.gz

export DOWN_LIST="hadoop_n_hbase_download_list.txt"
...
for uhost in `cat $DOWN_LIST`
 do
 echo $uhost
 tar -zxvf ${uhost}.tar.gz
 cd $uhost
 ant
 cd ..
done

echo "going to ln ...,^C to break"
read read_line
...
for host in `cat servers`
 ...
 echo "untaring on host:($host)"
 scp ${WGETDIR}/${HAD}.tar.gz ${WGETDIR}/${HBA}.tar.gz $host:~
 ssh -l $host 'mkdir -p /mnt/pkgs; cd /mnt/pkgs; tar xzf ~/${HAD}.tar.gz; tar xzf ~/${HBA}.tar.gz;sudo rm -fr /mnt/hadoop;sudo rm -fr /mnt/hbase; ln -s /mnt/pkgs/${HAD} /mnt/hadoop; ln -s /mnt/pkgs/${HBA} /mnt/hbase'
done
...
read read_line
export HADOOP_CLASSPATH=/mnt/hbase/${HBA}.jar:/mnt/hbase/${HBA}-test.jar:/conf
